...
export interface Prop { kind: 'palm' | 'lamp' | 'bench' | 'hydrant'; x: number; z: number; yaw: number; scale: number }
...
- `src/game/render/Renderer.ts`: `class Renderer { constructor(canvas, settings); scene; camera /* fov 65, near 0.3, far 900 */; gl; resize(); render(); applySettings(s); setFog(color: number, near: number, far: number); readonly drawCalls; readonly triangles; dispose() }`.
...
- Acceptance: typecheck + lint clean; `npm test` runs (0 tests is fine); `?autostart=1` renders ground + orange box; a Playwright screenshot shows it; no console errors.
...
**`TextureFactory`** (class, cached by key, all CanvasTexture, correct `colorSpace`/wrap/mipmaps): `windows(style, seed)` → `{ map, emissive }` 256×512, 4 columns × 8 rows per tile so one tile = 16 m × 28 m (UVs in meters: u = x/16, v = y/28; roofs get flat color via a 4-px plain strip at v > 0.99), lit-window pattern 35 % warm; `road()` 512×512 tile per 20 m (asphalt noise, dashed white lane separators at ±3.5 m, double yellow center); `crosswalk()`; `sidewalk()`; `sand()`; `water()`; `grass()`; `plaza()`; `neonAtlas(words)` one 1024×1024 atlas (rows of glowing text, returns per-word UV rects); `palmFrond()` alpha; `radialGlow()`; `starField()`; `policeLivery()`; `dispose()`.

**`Materials`**: MeshLambertMaterial for everything lit (buildings with `vertexColors`, `map` + `emissiveMap` windows, `emissive` white, `emissiveIntensity` = night factor), roads/sidewalk/sand/grass/plaza Lambert; water MeshPhongMaterial (shininess 80, opacity .92, uv scroll in `update(t)`); `carPaint()` Lambert white (instanceColor tints), `glass()`, `neon(atlasTex)` MeshBasicMaterial `fog=false toneMapped=false transparent` with opacity = night factor (min 0.15 by day); `lampHead()`, `lightPool()` additive MeshBasic `fog=false depthWrite=false`; `marker(color)` additive; `setNight(f)` drives all of the above. Expose `readonly nightFactor`.

**`BuildingGeometry`**: `buildingGeometry(b: Building): BufferGeometry` — box (or 2–3 stepped tiers for `stepped`, box + pyramid for `spire`) positioned at world coordinates, **UVs in meters** per face (u along the face width / 16, v = height / 28), vertex color = `b.color` (roof slightly darker). `landmarkGeometries(l: Landmark)` returns `{ geometry, style }[]` (tower: 3 setbacks + spire; arena: 32-segment elliptical cylinder with ribbed boxes; hospital: white box + red cross; police: box + blue band; lighthouse: striped cylinder; pier: planks + posts; ferris: torus + 12 spokes + 12 gondola boxes in a `Group` exposed separately for rotation).

**`CityRenderer`**: `constructor(scene, city, roads, materials, tex)`, `build()`, `update(time, nightFactor, playerX, playerZ)`, `dispose()`. Draw-call plan: buildings merged **per style** with `BufferGeometryUtils.mergeGeometries` (5 meshes, `frustumCulled` on, bounding spheres computed); landmarks ≤ 12 meshes; roads: one merged mesh of segment planes + one merged mesh of intersection squares (crosswalk texture); sidewalks one merged mesh of curb boxes (`CURB_H`); ground plane (dirt), park/plaza planes, beach sand plane, ocean plane 3000×3000 at y −0.3; palms 2 InstancedMeshes (trunk = 3 stacked slightly bent cylinders merged, fronds = 7 crossed alpha planes); lamps 3 InstancedMeshes (pole, head, additive light pool disc 12 m); neon signs: one merged mesh with the atlas material; ferris wheel group rotating 0.15 rad/s; lighthouse beam sprite rotating at night. Total static draws ≤ 40.

**`SkySystem`**: dome `SphereGeometry(850)` BackSide ShaderMaterial gradient (uTop, uHorizon, uSunDir, uSunColor, uStars) following the camera; sun + moon sprites; stars `Points` fading in at night; `DirectionalLight` (sun by day, moon .15 blue at night) with shadow camera 120 m box following the player (texel snapped, only on `high`); `HemisphereLight`; `scene.fog = THREE.Fog` with keys below; `update(hour, sunDir, nightFactor, playerX, playerZ, shadows)`. `SKY_KEYS` at hours 0, 4.5, 6, 7.5, 12, 16.5, 18, 19, 20.5, 22 with top/horizon/sun/fog colors, sun/ambient intensity, fogNear/fogFar (day 160/620, sunset 140/520, night 60/380). Sunset must read as Vice City: horizon `#ff7a3d`, top `#6a2c8f` around 18:30–19:15.
...
**`MarkerRenderer`**: pulsing additive cylinders (r 3, h 40, opacity .25–.5) for mission starts (orange) and the objective (blue) + a bobbing arrow cone above the objective; reads `world.mission` and the marker list via a `getMarkers(out)` callback provided by Engine (Track F's `MissionSystem.availableMarkers`).
...
Acceptance: screenshots at hours 12, 18.7, 22 look right (windows lit at night, neon on, lamps with light pools, sunset gradient); static draw calls ≤ 40; typecheck/lint clean.
...
**`CameraController`** (implements `CameraLike`; frame-rate update on interpolated transforms): `CAMERA_TUNING = { orbitDist: 5.5, orbitHeight: 1.6, pitchMin: -0.35, pitchMax: 1.1, pitchDefault: 0.22, mouseSens: 0.0022, keyYawSpeed: 2.4, chaseDists: [7.5, 11, 3.2], chaseHeight: 2.8, chaseLookAhead: 3, posLambda: 6, yawLambda: 4, autoAlignDelay: 1.0, autoAlignRate: 2.5, reverseDelay: 0.5, fovBase: 65, fovMax: 80, fovSpeedRef: 45, shakeDecay: 1.6, shakeMaxPos: 0.45, shakeMaxRot: 0.03, occlusionPad: 0.4, cinematicRadius: 140, cinematicHeight: 55, cinematicRate: 0.06 }`. Orbit on foot (mouse when locked, Q/R otherwise, `invertY`, `mouseSensitivity`); chase in vehicle (behind, auto-align after idle, reverse cam when reversing > 0.5 s, `V` cycles `chaseDists`, speed FOV); building occlusion pull-in via `staticHash.querySegment` + `segmentVsAabb`; trauma shake (`camera:shake` events + `addTrauma`); cinematic orbit around the tower in `menu`. `isInView` uses the current forward. `yawForMovement` = camera yaw.
...
**`VehicleRenderer`**: per-spec body InstancedMesh (5) with merged geometry (lower body + cabin with darker vertex colors for glass + bumpers, nose toward +Z), `instanceColor` = paint darkened by damage `(0.35 + 0.65·health/100)`, wrecks black; one wheels InstancedMesh (4 per vehicle, steer on front, spin); one lights InstancedMesh (headlight + taillight quads, instanceColor: white/red when `lightsOn`, dark otherwise; police bar quads alternate red/blue at 4 Hz while `sirenOn`); two SpotLights permanently in the scene for the player's headlights (intensity 0 by day); capacity `BUDGET.MAX_VEHICLES`; `spawnFade` scales instances; instances beyond 260 m collapsed (scale 0). ≤ 8 draws.
...
**`PedRenderer`**: 4 InstancedMeshes (body, head, legL, legR) capacity `BUDGET.MAX_PEDS`, instanceColor shirt/skin/pants, leg swing from `animPhase`, tumble/lying pose for HIT/DEAD, fade via `spawnFade`/`fadeTimer`, collapsed beyond 120 m.
