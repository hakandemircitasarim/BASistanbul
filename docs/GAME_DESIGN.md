...
Round 4 additions: `ParkedCar { x, z, yaw, spec: 'sedan'|'sport'|'van', colour }` and `LotProp { kind: 'planter'|'island'|'booth', x, z, yaw }`, exported on `CityData` as `parked?` and `lotProps?` (set dressing with colliders, never `Vehicle`s). `CityLots.furnishLots(ctx)` fills the lots after the gameplay spots: kerb islands with planters at both heads of every bay strip, a static car in ~40 % of the free bays (deterministic from the city seed); the gameplay spot, two bays on the driver's exit side and one on the far side, the gate's drive lane and the islands stay free; `CityBuild.addParkedCar/addLotProp` add the AABB colliders. Round 5: `Prop.kind` gains `'tree' | 'hedge'`, `ParkedCar.at: 'lot' | 'kerb'`; `CityProps.furnishStreets` (called from `CityLots.furnishLots`) adds deterministic kerbside parking **straddling the kerb into the kerb lane** (round 10 inverted the round-5 rule: the strip used to stop 8 cm SHORT of the asphalt and stand with its whole width on the 3 m footway, which blocked the pavement and read from the street as a car parked on the shop terrace; `KERB.kerbGap` is gone). The contract is now, all exported as `KERB_PARK` and asserted to the centimetre in `scripts/tests/city.test.ts`: the road-side flank stands at `SIDEWALK_W + KERB_PARK.roadLap` (3.0 + 0.45 = 3.45 m) out from the block edge, so the road-side wheels are on the asphalt and the kerb-side wheels up on the footway (~1.6 m of footway stays clear); the clearance from that flank to the outer traffic lane's body envelope is `KERB_PARK.laneClear` = 0.30 m, measured against `KERB_PARK.outerLaneOff` = 4.75 m (the outer lane centre out from the block edge) and `KERB_PARK.bodyHalf` = 1.0 m (half the widest traffic body, the van); cars stay outside intersections, crossings and every furniture rect, and `KERB.pointClear` is **11 m**, not 3.5, because a car dressing the kerb lane beside a named point stands in the lane the gameplay vehicle spawning there has to pull out into. Renderer-side half of the contract: `CityRendererProps.kerbStance` samples `ContactShadows.groundYAt` under both wheel lines and rolls the body by the difference (~5.5 deg), putting the contact blob on the HIGHER of the two surfaces — so `groundYAt` must answer `CURB_H` on all four sidewalk aprons of a block (it indexes the grid with the apron folded in, `Math.floor((x - ROAD_W + SIDEWALK_W) / PITCH)`; without that the low-x and low-z aprons answered 0 and half the kerb cars stood flat with their kerb-side half buried in the pavement). Traffic-side: the siren yield is clamped on the outer lane to `KERB_PARK.laneClear` (`TrafficSystem.kerbSafeYieldOffset`) so a yielding car cannot be steered into the parked row. `furnishStreets` also adds round-crown sidewalk trees on downtown/suburb blocks; `CityLots.addLotHedges` screens lot edges from the street.
...
// Collider-vs-lane invariant: every non-water/boundary collider must clear a lane centre line by LANE_W / 2 (1.75 m) - the only automated guard against
// generating something solid into a driving lane. The ONE exemption is a kerbside parked car's footprint, recognised by a kerb ParkedCar standing exactly at
// the collider's centre and held to KERB_PARK.bodyHalf instead; the extra KERB_PARK.laneClear is asserted in city.test.ts. Never weaken the rule globally.
```
...
- `src/game/render/Renderer.ts`: `class Renderer { constructor(canvas, settings); scene; camera /* fov 56, near 0.3, far 900 */; gl; resize(); render(); applySettings(s); adapt(fps, frameDt); setBloomForNight(nightFactor); setGrade(nightFactor, duskFactor, time); sceneBreakdown(); readonly drawCalls; readonly triangles; dispose() }`. On `high` the frame goes through an EffectComposer on a 4x MSAA HalfFloat target: RenderPass → UnrealBloom (HDR-only: threshold 3.0 by day, 1.4 at night) → OutputPass (ACES) → grade (saturation 0.92, lifted toe, split tone that peaks at dusk, fine grain). Fog is owned by `SkySystem`.
  `powerPreference 'high-performance'`, antialias on 'high', pixelRatio min(devicePixelRatio, high 1.5 / low 1), `outputColorSpace = SRGBColorSpace`, ACESFilmic tone mapping exposure 1.0, shadows only on high (`THREE.PCFShadowMap` - in three r185 that IS the soft filter, PCFSoftShadowMap is deprecated and falls back to it - at `SKY_TUNING.shadowMap` = **3072**, which costs ~75.5 MB of VRAM, see SkySystem below). Resize observes the canvas parent.
...
**`CityRenderer`**: `constructor(scene, city, roads, materials, tex)`, `build()`, `update(time, nightFactor, playerX, playerZ)`, `dispose()`. Draw-call plan: buildings merged **per style** with `BufferGeometryUtils.mergeGeometries` (style meshes + trim (receiveShadow off) + glow + shop band + plinth band + awning mesh; `frustumCulled` on, bounding spheres computed); `streetFaces()` bitmask per building (faces within `STREET.bandMaxSetback` of the block edge) decides where relief, columns and balconies go; landmarks ≤ 12 meshes; roads: one merged mesh of segment planes + one merged mesh of intersection squares (crosswalk texture) + road-mark decals; sidewalks one merged mesh of curb boxes (`CURB_H`), lots with kerbs; ground plane (dirt), park/plaza planes, beach sand plane, ocean plane 3000×3000 at y −0.3; props via `CityRendererProps` (one InstancedMesh per part, 14 draws): palms 2 seeded variants (lean/height) with fronds of 6 rows drooping parabolically from the base on a 0.25 m V midrib, 14-16 live fronds with jittered yaw/pitch, a fibrous bulb crown and a small dead skirt; lamps (10-sided pole with base collar, 4-station swept gooseneck arm, rounded head, additive light pool + facade spill quads); slatted benches with cast-iron ends; chamfered 12-16-segment bins / bollards / hydrants; rounded-blade street signs; bus shelters (round posts, rounded roof slab with fascia lip, thin-framed `glass()` panes as their own instanced mesh, slatted seat). `PROP_RANGE` packs only nearby instances and repacks after 15 m of camera movement. Neon signs: one merged mesh with the atlas material; ferris wheel group rotating 0.15 rad/s; lighthouse beam sprite rotating at night. Static draws ≈ 52 at noon; whole frame < 120. Neon/wall signs (`buildNeon`) sit on a dark backing box 0.25 m proud of the wall with two brackets and the glyph quad 5 cm in front; the emissive multiplier is ~1.5 (large signs dimmer) so letters stay legible instead of blooming. Round 5 props (`CityRendererProps`): per-instance y, near/far LOD split with shared-mesh cursors, tree (trunk lathe + faceted crown, `foliage` material), hedge units, kerb parked shells (`PROP_RANGE.kerb` 135, `tree` 120, `hedge` 100) and a flat-shaded solid palm-frond LOD beyond 60 m. **`BackdropRenderer`** (new): a lit, fogged ring of low hills plus a ground skirt at ~1.5× the city radius in `materials.plain`, one merged mesh, constructed by Engine and RenderTest, `dispose()` only. **`FacadeDetailRenderer`** (round 6, wired from CityRenderer, +1 draw): window frames (head + jambs 0.26 m proud, dark reveals) and sills, railed/solid balconies, AC units and downpipes as unit geometries in ONE `THREE.BatchedMesh` (`materials.facade`, `perObjectFrustumCulled`, `sortObjects = false`), packed for buildings within `FACADE_RANGE` of the player and repacked after 15 m of movement without allocating (per-cell geometry ids are precomputed); back-facing walls and cells above the 6th floor beyond 30 m are skipped. `FACADE_RANGE` = `{ window: 55, balcony: 90, ac: 45, pipe: 70, wallKit: 60, joint: 42, highWindow: 30 }` and `CELL_KIND` has **six** kinds, not four: round 12 added `wallKit` (one string course) and `joint` (a raised expansion-joint strip) as the minimum kit for the elevations the street relief skips — size `MAX_UNITS` (3000) and the batch's vertex budget against all six. **Round 13, the rule those two live under: no face of any building is blank.** `facade()` maps the windowed tile onto every face, street or not, so the painted floor lines (world y = a multiple of `Massing.rowH` ∈ {2.94, 3.43, 3.92, 6.86} — never `FLOOR_H` 3.5) and the painted bay lines (multiples of `Massing.bayW` from the face's start corner) run behind the kit, and any kit piece not placed on that grid cuts glazing. `blankWallKit` therefore snaps the course to a floor line (its exact world y travels on the unit's own position, so the batch needs ONE course geometry) and every joint to a bay line, and emits **no plinth at all** — the tile's ground row (big shop panes) starts 0.02 of a row above the wall foot, so no plinth height is safe. Requires `WEBGL_multi_draw` (three 0.185 has no fallback). Bay lines and stop bars use the dedicated `MARK_UV.line/stop` atlas strips (no mip bleed from the arrow cells); `NEON.backing` is 0.42 m so the floor slabs never z-fight the sign box. Round 6: every prop part that shares a material lives in ONE `THREE.BatchedMesh` per material (furniture, foliage, parked shells, palm trunks, palm fronds — five batches; `setVisibleAt` does the PROP_RANGE culling, per-instance colour keeps the parked paint), hedges are 3-4 variants with noisy tops joined into low continuous runs with colour/scale jitter, tree crowns come from 3 seeds with 4-6 lobes, scale 0.8-1.3 and hue jitter at irregular spacing (`CityProps`). Round 7-9 — the prop pass is no longer the five batches above: the static parked cars are a three-tier ladder over **seven** batches, `prop:parkedNear` holding `NEAR_CARS.cap` = **3** cars within `NEAR_CARS.range` = **8 m** at the full `parkedNearGeometry` body loft and `prop:parkedMid` holding `MID_CARS.cap` = **16** within `MID_CARS.range` = **32 m** at `parkedMidGeometry` (~980 tris: real arch fenders, greenhouse pillars, lamp cells, 12-sided shouldered tyres), with the coarse shell of every car either band took hidden while it is up. The coarse shell itself carries no fake arch band any more (removed round 9: 64 tris a car paid three times a frame for a crescent that is sub-pixel past 60 m). Both capped bands are re-picked on their OWN cadence, `CAR_TIER_MOVE` = 1.5 m of camera travel, not on the 15 m `PROP_RANGE.repackMove` — a band 8 m wide cannot be assigned on a 15 m cadence — and a re-pick hands the previous picks' coarse shells back first, so nothing is ever drawn twice or lost; the utility-pole catenaries are one `THREE.LineSegments` repacked by range, the lamp's facade spill quad is GONE, and `PropRenderer` adds `LAMP_LIGHTS.count` = **3 permanent `THREE.PointLight`s** to the scene (140 cd, distance 30, decay 2, colour 0xffc48a, at y 6.4). They are never removed — only their intensity is ramped with the night factor (a light leaving the scene recompiles every lit material), so they sit in `NUM_POINT_LIGHTS` at every hour, noon included: budget any further light against **3 point lights + the 2 headlight SpotLights**. They are re-aimed at the nearest lamp heads every frame (not on the 15 m repack) and fade out over the last third of their range, and every in-range lamp keeps its additive pavement disc, so a lamp handed from one light to another never changes a pixel in one frame. Round 9-10: the static parked cars also get blob contact shadows from the shared `ContactShadows` field (`CAR_SHADOWS`, range 46 m) — a parked car is not a `Vehicle`, so it had none and floated once its own sun shadow was a few texels. The cap is **44**, sized so it CANNOT bind (the densest 46 m neighbourhood in the generated city holds 40 parked cars; with the old cap of 24 the pick was a rank cut and neighbouring bays in a lot at 15 m blinked between grounded and floating every 1.5 m of camera travel). `SHADOW_TUNING.capacity` must cover every slice reserved from that one mesh — vehicles 96 + peds `MAX_PEDS` + player 1 + parked cars `CAR_SHADOWS.cap` (44) + street furniture `FURN_SHADOWS.cap` (24, round 13, and the LAST to reserve, so the one a shortfall actually clamps) = 261 against a capacity of 320 — or the last renderer built silently gets fewer blobs (guarded by a test in `render.test.ts`, which reads both caps from `CityRendererProps` rather than repeating the numbers). Round 11-13, the blob itself: it is a **MULTIPLY**, not an alpha-over — `blendSrc ZeroFactor` / `blendDst OneMinusSrcAlphaFactor`, so the mask's alpha is the OCCLUSION FRACTION and the material has no colour at all (`SHADOW_TUNING.color` no longer exists). `dayOpacity` 0.5 / `nightOpacity` 0.62 are therefore fractions of the light under the caster and **cannot be compared with the old alpha-over numbers**. The mask is no longer a texture and there is no `core` fraction and no `texSize`: it is a rounded-box distance field evaluated per fragment from the instance's own world scale (`blobMaskPatch`), so the sizing is in METRES — `contactExtent(half) = max(half + SHADOW_TUNING.spill (0.26) + SHADOW_TUNING.penumbra (0.34), minHalf 0.5)`, undiluted occlusion out to `spill` past the caster's own footprint and `penumbra` of soft edge outside that, the same world widths on all four sides whatever the quad's aspect. **Every** caster goes through it: `VehicleRenderer.shadowExtent` IS `contactExtent` (moving and parked cars), `PED_RENDER.shadowR` = `contactExtent(PED_FOOT_HALF 0.22)` = **0.82** and `PlayerRenderer.SHADOW_R` = `contactExtent(PLAYER_FOOT_HALF 0.25)` = **0.85** — derived, not tuned numbers, which is what the `render.test.ts` shape test asserts (core = extent − penumbra must clear the footprint by 0.2 m, and the extent must stay within footprint + 1 m). Two per-instance scalars ride on `instanceColor` and are read in the FRAGMENT stage behind **`#ifdef USE_COLOR`**: `.r` the spawn / cull fade (it scales the occlusion, not only the matrix) and `.g` the roundness (`add(..., round)`; 0 = the rounded rectangle a car's footprint wants, 1 = the ellipse a standing figure wants, and a figure given the rectangle stands in a visible rounded square). **`USE_INSTANCING_COLOR` is a VERTEX-ONLY define in three 0.185** — the fragment prefix answers an instanceColor attribute with `USE_COLOR`, which is also what `color_pars_fragment` declares `vColor` under — so guarding the fragment half on it compiles both scalars away in silence (round 12's bug, found by reading `gl.getShaderSource` off the live program; `render.test.ts` now asserts the patched fragment source never names the vertex-only define). A blob laid along a caster's yaw must put the LONG half-extent in `rz`: `add` composes the quad on a plane rotated by yaw, so local +Z is the yaw direction, and a body lying on the ground lies ALONG its yaw (PedRenderer's `'YXZ'` pose tips local +Y onto −Z before the yaw turn; `PlayerRenderer` sets `group.rotation.order = 'YXZ'` for the same reason). The mesh's `renderOrder` is **1.5**: above the ground paint (road marks and skid marks at 1), which writes no depth and would otherwise be painted back over the multiply at full brightness, and below every additive (lamp pools 2, neon 3, markers 4, particles 5-6), which must add on top of the shaded floor.

**`SkySystem`**: HDR dome `SphereGeometry(850)` BackSide ShaderMaterial (three-stop gradient, `uExposure` 1.8 by day → 1.0 at night, sun disc bright enough to bloom, two-octave cloud layer projected on a flat plane, stars) following the camera; sun + moon sprites; `DirectionalLight` (warm sun ≈ 4.5 at noon, moon 0.5·L `#b8c4e8` with moon shadows at night) with a **132 m** shadow box following the player (texel snapped 0.043 m, `shadow.bias` **-0.00045**, `normalBias` **0.035** — DERIVED from the texel, 0.8 of one, so it must be re-derived whenever `shadowMap` changes — `SHADOW_TAPS` = 7 Vogel-disk taps over a `SHADOW_RADIUS` = **6** texel disk — 4.30 cm a texel at 132 m over **3072**, so 0.26 m at full width — only on `high`. The map is 8 bytes a texel, not 4: three gives a PCF directional shadow both an RGBA8 colour attachment it never samples and a 32-bit depth texture, i.e. 33.6 MB at 2048, **75.5 MB at 3072**, 134.2 MB at 4096). `SHADOW_CONTACT` no longer exists: `patchShadowPenumbra` now scales that disk through the three-rung ladder `SHADOW_PENUMBRA = { nearM: 3.5, midM: 14, near: 0.18, mid: 0.42 }` from **eight** blocker probes, i.e. 1.1 texels (4.7 cm) under anything within 3.5 m (a figure, a wheel, a kerb, a bollard), 2.5 texels (11 cm) for palm crowns and first-floor parapets and the full 6 texels (26 cm) for a facade thrown down the street. The probes are pinned to the shadow map's axes and there are four a rung (±U and ±V, max), which is a round-13 fix and not decoration: the probe decides whether the disk widens at all, a probe pinned to U alone can only cross an edge that runs across U, and U is perpendicular to the sun's azimuth — the direction a shadow's TIP edge runs — so a building's long dusk shadow came out hard on its lit side and soft on its dark one, and which edges were hard rotated with the sun through the day. They are pinned rather than spun by `phi` because a per-fragment probe angle dithers the penumbra WIDTH. Round 12's `SHADOW_DITHER = { fromTexels: 2, spanTexels: 3 }` is the other half of that: the per-pixel rotation of the TAP disk (`tapPhi = phi * clamp((softRadius / texel − 2) / 3)`) is ramped in with the disk's width, because three rotates the Vogel disk by an interleaved-gradient noise of `gl_FragCoord` on every fragment and at seven taps over a narrow disk that is an ordered, screen-stable crosshatch on every soft edge in the frame. **`nearM`/`midM` are metres converted against `SHADOW_DEPTH_SPAN` = 360 and `SHADOW_CAM_NEAR` = 60, both exported from SkySystem and both the shadow camera's own near/far (`sc.near = SHADOW_CAM_NEAR`, `sc.far = SHADOW_CAM_NEAR + SHADOW_DEPTH_SPAN`): setting a far plane that is not derived from those two silently rescales the whole ladder and every caster falls into the widest disk** — which was exactly the round-9 failure (a single 1.5 m step, shorter than a person, put every caster taller than a kerb in the wide bucket, so nothing read as a shape). The box's hard stair-stepped border is hidden by `patchShadowEdgeFade()`, a one-time patch of three's `shadowmap_pars_fragment` that fades the shadow term back to lit over the outer `SHADOW_FADE` = 10 % of the box, NOT by making the box bigger: the box is the shadow pass' caster set and every metre of it is paid in the colour, GTAO and shadow passes (measured: 150 m cost ~9.5k triangles more than 120 m in the dusk frame); `HemisphereLight` driven by the key's `fill`/`ground` complements (≈ 1.5 at noon, key > fill); `scene.fog = THREE.FogExp2` owned here (`density = key.fogDensity / fogScale`, colour derived from the sky); a 128×64 float equirect reflection probe (graded sky + sun blob + horizon line) PMREM-filtered into `scene.environment` when the hour moves > 0.3 h; `update(hour, sunDir, nightFactor, playerX, playerZ, shadows)`. `SkyKey = { hour, top, horizon, sun, fill, ground, fog, sunI, ambI, fogDensity }`; `SKY_KEYS` at hours 0, 4.5, 6, 7.5, 12, 16.5, 17.5, 18, 19, 20.5, 22. Sunset must read as Vice City: horizon `#ff7a3d`, top `#6a2c8f` around 18:30–19:15. The sun path (`DayNightSystem`) leans south as it climbs so noon elevation is ≈ 49° while sunrise/sunset azimuth and the dusk timing are unchanged.
...
**`EffectsRenderer`**: skid marks ring buffer (3000 quads, `DynamicDrawUsage`, `drawRange`) fed by `fx:skid` events + per-vehicle rear-wheel positions; tire smoke and sparks as `Points` with life attribute (400 + 300); damage smoke for vehicles with `smoke > 0`; explosion flash sphere on `vehicle:destroyed`; listens to `vehicle:collision` (sparks ∝ impactSpeed — `clamp(impactSpeed * 3, 4, 40)`, but **gated below `FX_TUNING.sparkMinImpact` = 1.5 m/s**, which throws no sparks at all; the 4-spark floor otherwise fired on brushes the player cannot see or hear, and since the parked strip moved into the kerb lane the AI grazes a parked flank at ~0.1 m/s hundreds of times a minute city-wide — 738 such events in a 600 s headless sweep, mean impact 0.14 m/s, of which the gate passes 5) and `ped:hit` (dust). `update(world, alpha, frameDt)`.
...
Acceptance: screenshots at hours 12, 18.7, 22 look right (windows lit at night, neon on, lamps with light pools, sunset gradient); static draws ≈ 52 and the whole frame < 120 draw calls; triangles ≤ 720k measured at the **worst of {spawn, 20 m north, 40 m north} × {12:00, 19:00, 21:00}** — NOT at the spawn frame, which is not the worst case: round 8 passed a spawn-frame check and still breached at 742-747k twenty and forty metres up the street. Round 11 re-measured all nine (`scratchpad/pw/r10gate.mjs <base> <hour>`, 1280×720, `quality=high&noadapt=1`, clock pinned, 45 s population dwell): **698,242 triangles / 111 draw calls at 19:00 / 20 m north** — the binding POSITION moved from 40 m to **20 m** north at both 19:00 and 21:00, so sampling only the old point measures the wrong frame. Spawn – 20 m – 40 m: 12:00 640.2k/105 – 669.6k/107 – 646.4k/107; 19:00 668.1k/109 – 698.2k/111 – 682.0k/111; 21:00 666.2k/108 – 696.7k/110 – 679.5k/110. Headroom is **21.8k triangles**, not the ~50k round 10 recorded; a ped is **1,926** triangles (not 1,882) and pays them twice, so 40 of them are 154.1k, a fifth of the whole frame; typecheck/lint clean; every visual change is judged from a screenshot.
...
**`PedRenderer`** (round 13: TWO TIERS, **7** InstancedMeshes): six near parts (body, head, legL, legR, armL, armR) named `ped:part`, all capacity `BUDGET.MAX_PEDS`, plus ONE merged far figure `ped:far` at the same capacity. A ped inside `PED_RENDER.lodDist` (40 m from the camera, with `lodHyst` 1.5 m of hysteresis) is the six near parts, **1,990 triangles**, and casts the sun shadow; past it, one `ped:far` instance of **432 triangles** that does NOT cast (measured: turning that on changes ~100 px of a 1280×720 frame and costs 432 tri per ped and 1 draw — see the comment at the `far.castShadow` line). instanceColor shirt/skin/pants plus a per-vertex `pedPants` flag that paints the torso tube below the belt in the TROUSER colour, leg swing from `animPhase` (the far tier swings from one per-instance value in its own vertex shader), tumble/lying pose for HIT/DEAD, fade via `spawnFade`/`fadeTimer`, DROPPED from the instance list (not scaled to zero) beyond `PED_RENDER.cullDist` = **96 m**, with a `cullFade` 6 m ramp. `Pedestrian.renderIndex` is an index WITHIN the ped's own tier, not into one mesh.
